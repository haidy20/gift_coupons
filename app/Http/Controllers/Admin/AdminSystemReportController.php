...
class AdminSystemReportController extends Controller
{
    ...
    public function AdminGetAllVouchers()
    {
        ...
        if (auth()->user()->role !== 'superAdmin') {
            return response()->json([
                'status' => false,
                'message' => 'Unauthorized. only superAdmin can see all the vouchers in the system',
                'data' => null
            ], 403);
        }
    ...
    public function AdminGetAllStatusVouchers(Request $request)
    {
        if (auth()->user()->role !== 'superAdmin') {
            return response()->json([
                'status' => false,
                'message' => 'Unauthorized. only superAdmin can see all the status of vouchers in the system',
                'data' => null
            ], 403);
        }
        ...
        return response()->json([
            'status' => true,
            'message' => "Vouchers with status '{$status}' retrieved successfully",
            'data' => [
                'status' => $status,
    ...
    public function getAllUsersByRole()
    {
        ...
        if (auth()->user()->role !== 'superAdmin') {
            return response()->json([
                'status' => false,
                'message' => 'Unauthorized. only superAdmin can see all roles',
                'data' => null
            ], 403);
        }
